...
// Track the number allocated so you know the next entry of the_nodes 
// that is available, and can check for trying to allocate more than 
// NUM_NODES nodes.
...
void bst_add(struct bst_node **proot, char *str) {
  // Fill this function in Don't forget, proot is a _pointer to_ the 
  // pointer to the BST root. This is so that when a new subtree is 
  // needed, you can set *proot. Modifying a caller's variable in this 
  // way is something not available in Java and many other languages, 
  // but is a useful technique in C. Note that, to access the count 
  // field, for example, you need to write (*proot)->count, etc.
  if (*proot == NULL) {
    // Insert code here to allocate a new bst_node struct from the 
    // array. If no more space is available, you should print "Out of 
    // space!\n" and call exit(1); If you _can_ get a node, fill in its 
    // fields and set root (what proot points to!) to point to it.  
    // Don't forget to copy str using strdup().
    //
    ...
    //
    // Be sure to initialize ALL the fields of the new node!
       if (num_allocated >= NUM_NODES)
       {
        printf("Out of space!\n"); 
        exit(1);
       }
    ...
    int cmp = strcmp(str, (*proot)->string);
      if (cmp == 0) {
      // Insert code here to increment to count of the bst_node that 
      // root points to (root is what proot points to!).  One line of 
      // code will suffice.
	(*proot) -> count += 1;
    } else if (cmp < 0) {
      // Insert code here to call bst_add on the 'left' field of the 
      // bst_node that root points to.  (Recall, root is what proot 
      // points to!)  To do this, you need need to get the _address_ of 
      // the 'left' field of the struct.  Again, one line of code will 
      // suffice.
