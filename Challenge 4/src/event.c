...
struct Time elapsed_time(struct Event e1, struct Event e2) {
  ...
  struct Time ret;
  int total = totalFromHMS(e1.st.hour, e1.st.min, e1.st.sec) - totalFromHMS(e2.st.hour, e2.st.min, e2.st.sec);
  if (total < 0)
  {
      total = total * -1;
  }
  ret.hour = totalToHours(total);
  ret.min = totalToMinutes(total);
  ret.sec = totalToSeconds(total);
  return ret;
}
